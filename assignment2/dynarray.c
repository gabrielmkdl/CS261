...
/*
 * This function returns the start of the array 
 *
 * Params:
 *   da - the dynamic array that we want the start of
 */
 ...
 *
 * Params:
 *   da - the dynamic array that we want the size of
 */
 ...
 *
 * Params:
 *   da - the dynamic array that we want the first element of
 */
void* get_front(struct dynarray* da) {
...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    int physical = (da->start + i) % da->capacity;
    new_data[i] = da->data[physical];
  }
...
void* dynarray_remove(struct dynarray* da) {
  ...
  void* temp = da->data[da->start];
  da->data[da->start] = NULL;
  
  if(da->capacity == da->start + 1) {
    da->start = 0;
  }
